...
ros::Publisher pubLoopScanLocalUpdate;
...
pcl::PointCloud<PointType>::Ptr frametrans(const pcl::PointCloud<PointType>::Ptr &cloudIn, const Eigen::Matrix4d & transCur)
{
    pcl::PointCloud<PointType>::Ptr cloudOut(new pcl::PointCloud<PointType>());
...
std::optional<gtsam::Pose3> doICPVirtualRelative( int _loop_kf_idx, int _curr_kf_idx )
{
    ...
    loopFindNearKeyframesCloud(cureKeyframeCloud, _curr_kf_idx, 1, _loop_kf_idx); // use same root of loop kf idx 
    ...
    icp.setInputTarget(targetKeyframeCloud);// Btarget_T_Bsource
    pcl::PointCloud<PointType>::Ptr unused_result(new pcl::PointCloud<PointType>());
    Eigen::Matrix4d initT = Eigen::MatrixXd::Identity(4,4);
    icp.align(*unused_result, initT.cast<float>());
    initT = icp.getFinalTransformation().cast<double>();

    // // parse pointclouds
    // historyKeyframeSearchNum = 1; // enough. ex. [-25, 25] covers submap length of 50x1 = 50m if every kf gap is 1m
    ...
    // if (icp.getFitnessScore() > loopFitnessScoreThreshold) {
        std::cout << "[SC loop] ICP fitness test failed (" << icp.getFitnessScore() << " > " << loopFitnessScoreThreshold << "). Reject this SC loop." << std::endl;
        std::cout << "***************************" << std::endl;
        return std::nullopt;
    } else {
        loopFindNearKeyframesCloud1(cureKeyframeCloud, _curr_kf_idx, 0, _curr_kf_idx); // use same root of loop kf idx 
        loopFindNearKeyframesCloud1(targetKeyframeCloud, _loop_kf_idx, historyKeyframeSearchNum, _loop_kf_idx);
        pcl::PointCloud<PointType>::Ptr cureKeyframeCloudUpdate(new pcl::PointCloud<PointType>());
        cureKeyframeCloudUpdate = frametrans(cureKeyframeCloud, icp.getFinalTransformation().cast<double>());

        sensor_msgs::PointCloud2 cureKeyframeCloudUpdateMsg;
        pcl::toROSMsg(*cureKeyframeCloudUpdate, cureKeyframeCloudUpdateMsg);
        cureKeyframeCloudUpdateMsg.header.frame_id = "camera_init";
        pcl::toROSMsg(*cureKeyframeCloud, cureKeyframeCloudMsg);
        cureKeyframeCloudMsg.header.frame_id = "camera_init";
        pcl::toROSMsg(*targetKeyframeCloud, targetKeyframeCloudMsg);
        targetKeyframeCloudMsg.header.frame_id = "camera_init";
        ...
        pubLoopScanLocalUpdate.publish(cureKeyframeCloudUpdateMsg);
    ...
    correctionLidarFrame = icp.getFinalTransformation();  // Btarget/loop_T_Bsource/current
...
void performSCLoopClosure(void)
{
    ...
    // // test publish
    // pcl::PointCloud<PointType>::Ptr cureKeyframeCloud(new pcl::PointCloud<PointType>());
    // loopFindNearKeyframesCloud(cureKeyframeCloud, keyframePoses.size() - 1, 0, keyframePoses.size() - 1); // use same root of loop kf idx 
    // sensor_msgs::PointCloud2 cureKeyframeCloudMsg;
    // pcl::toROSMsg(*cureKeyframeCloud, cureKeyframeCloudMsg);
    // cureKeyframeCloudMsg.header.frame_id = "camera_init";
    // pubLoopScanLocal.publish(cureKeyframeCloudMsg);
...
int main(int argc, char **argv)
{
    ...
    pubLoopScanLocalUpdate = nh.advertise<sensor_msgs::PointCloud2>("/loop_scan_local_update", 100);
