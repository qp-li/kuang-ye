...
double historyKeyframeSearchRadius = 10;
...
pcl::PointCloud<PointType>::Ptr global2local(const pcl::PointCloud<PointType>::Ptr &cloudIn, const Pose6D& tf)
{
    ...
    transCur.matrix() =pcl::getTransformation(tf.x, tf.y, tf.z, tf.roll, tf.pitch, tf.yaw).matrix().inverse();
...
void savePath(void)
{
    ...
    fout_out.open("/home/wuhan2021/00000testdata/data/result.txt", std::ios::out);
    std::cout<<"start savepath!"<<std::endl;
    for (int node_idx=0; node_idx < int(keyframePosesUpdated.size()) - 1; node_idx++) // -1 is just delayed visualization (because sometimes mutexed while adding(push_back) a new one)
    {
        const Pose6D& pose_est = keyframePosesUpdated.at(node_idx); // upodated poses
        geometry_msgs::Quaternion q4 = tf::createQuaternionMsgFromRollPitchYaw(pose_est.roll, pose_est.pitch, pose_est.yaw);
        Eigen::Quaterniond q4_ = Eigen::Quaterniond(q4.w, q4.x, q4.y, q4.z) ;//* Eigen::Quaterniond(Eigen::Vector4d(0.707105451466, 0.707108048814, 0.000209535067884, 0.00020953429822));
        fout_out << std::fixed << std::setprecision(6) << ros::Time().fromSec(keyframeTimes.at(node_idx)) << " " << std::setprecision(15) 
            <<pose_est.x <<" "
            <<pose_est.y <<" "
            <<pose_est.z <<" "
            << q4_.coeffs()(0)<< " " 
            << q4_.coeffs()(1) << " "
            << q4_.coeffs()(2) << " " 
            << q4_.coeffs()(3) << std::endl;
            // << q4.x<< " " 
            // << q4.y << " "
            // << q4.z << " " 
            // << q4.w << std::endl;
        if (node_idx == int(keyframePosesUpdated.size()) - 2)
        {
            std::cout<<"CE: "<< std::fixed << std::setprecision(6)
                    << sqrt(pose_est.x * pose_est.x + pose_est.y * pose_est.y) << " "
                    << abs(pose_est.z) << std::endl;
        }
    }
    fout_out.close();
    std::cout<<"end savepath!"<<std::endl;
} // savePath
...
void loopFindNearKeyframesCloud( pcl::PointCloud<PointType>::Ptr& nearKeyframes, const int& key, const int& submap_size, const int& root_idx)
{
    ...
    for (int i = -submap_size; i <= submap_size; ++i) {
        ...
        mKF.lock(); 
        // W2 p^B(keynear),    T_W1_B(root),   T_B(root)_B(currentkey)
        // p^B = T_B_A * p^A
        // so points should be in B(root) and B(currentkey)
        // for current keyframe, p^B(current)
        // for loop keyframes, T_W_B(root).inv() * T_W_B(keynear) * p^B(keynear) = p^B(root)
        // local2global(p^B, T_WB.inv() * T_WB(keynear))
        // so ICP returns T_B(root)_B(current)

        // odometry T_B1_B2
        // *nearKeyframes += * local2global(keyframeLaserClouds[keyNear], keyframePosesUpdated[root_idx]);
        // *nearKeyframes += * local2global(keyframeLaserClouds[keyNear], keyframePosesUpdated[keyNear]); // to global
...
void loopFindNearKeyframesCloud1( pcl::PointCloud<PointType>::Ptr& nearKeyframes, const int& key, const int& submap_size, const int& root_idx)
{
    // extract and stacking near keyframes (in global coord)
    nearKeyframes->clear();
    for (int i = -submap_size; i <= submap_size; ++i) {
...
std::optional<gtsam::Pose3> doICPVirtualRelative( int _loop_kf_idx, int _curr_kf_idx )
{
    ...
    loopFindNearKeyframesCloud(cureKeyframeCloud, _curr_kf_idx, 2, _loop_kf_idx); // use same root of loop kf idx 
    ...
    ros::NodeHandle nh;
    nh.param<float>("loopFitnessScoreThreshold", loopFitnessScoreThreshold, 2); 
    if (icp.hasConverged() == false || icp.getFitnessScore() > loopFitnessScoreThreshold) {
        std::cout << "[SC loop] ICP fitness test failed (" << icp.getFitnessScore() << " > " << loopFitnessScoreThreshold << "). Reject this SC loop." << std::endl;
        return std::nullopt;
    } 
    // else {
    //     loopFindNearKeyframesCloud1(cureKeyframeCloud, _curr_kf_idx, historyKeyframeSearchNum, _curr_kf_idx); // use same root of loop kf idx 
    //     loopFindNearKeyframesCloud1(targetKeyframeCloud, _loop_kf_idx, 2, _loop_kf_idx);
    ...
    loopFindNearKeyframesCloud1(cureKeyframeCloud, _curr_kf_idx, historyKeyframeSearchNum, _curr_kf_idx); // use same root of loop kf idx 
    // cureKeyframeCloud = transformPointCloud(cureKeyframeCloud, poseFrom);
    loopFindNearKeyframesCloud1(targetKeyframeCloud, _loop_kf_idx, 2, _loop_kf_idx);
    pcl::toROSMsg(*cureKeyframeCloud, cureKeyframeCloudMsg);
    cureKeyframeCloudMsg.header.frame_id = "camera_init";
    pcl::toROSMsg(*targetKeyframeCloud, targetKeyframeCloudMsg);
...
void process_pg()
{
    while(1)
    {
		while ( !odometryBuf.empty() && !fullResBuf.empty() )
        {
            ...
            thisPose3D.x = pose_curr.x;
            thisPose3D.y = pose_curr.y;
            thisPose3D.z = pose_curr.z;
...
void performSCLoopClosure(void)
{
    ...
    if( SCclosestHistoryFrameID != -1 ) { 
        ...
        if (abs(curr_node_idx - prev_node_idx) < 200)
        {
            cout << "Loop invalid!  " << prev_node_idx << " ---- " << curr_node_idx << "" << endl;
            return;
        }

        cout << "Loop valid!  " << prev_node_idx << " ---- " << curr_node_idx << "" << endl;
...
int detectLoopClosureID_EulDist(void){

    if( int(keyframePoses.size()) < 30) // do not try too early 
        return -1;

    // 当前关键帧帧
    int loopKeyCur = cloudKeyPoses3D->size() - 1; //  当前关键帧索引
    int loopKeyPre = -1;
    ...
    std::cout<<"(int)pointSearchIndLoop.size(): "<<(int)pointSearchIndLoop.size()<<std::endl;
    for (int i = 0; i < (int)pointSearchIndLoop.size(); ++i)
    {
        int id = pointSearchIndLoop[i];
        if (abs(keyframeTimes[id] - keyframeTimes.back()) > historyKeyframeSearchTimeDiff)
        {
            loopKeyPre = id;
            break;
        }
...
void performEDLoopClosure(void)
{
    // eul_dist
    int closestHistoryFrameID = detectLoopClosureID_EulDist();
...
} // performEDLoopClosure

void visualizeLoopClosure()
{
    ros::Time timeLaserInfoStamp = ros::Time().fromSec(timeLaserOdometry); //  时间戳
    ...
    for (int node_idx=0; node_idx < recentIdxUpdated; node_idx++) {
        if(counter % SKIP_FRAMES == 0) {
            *laserCloudMapPGO += *local2global(keyframeLaserClouds[node_idx], keyframePosesUpdated[node_idx]);
            // *laserCloudMapPGO += *local2global(keyframeLaserClouds[node_idx], keyframePoses[node_idx]);
        }
        counter++;
    }
    mKF.unlock(); 
...
int main(int argc, char **argv)
{
    ...
    nh.param<double>("historyKeyframeSearchRadius", historyKeyframeSearchRadius, 10);  
    ...
    float filter_size = 1; 
    nh.param<float>("sc_icp_filter_size", filter_size, 1); // pose assignment every k frames 
    ...
    pubLoopConstraintEdge = nh.advertise<visualization_msgs::MarkerArray>("/loop_closure_constraints", 100);
