...
void ImuProcess::IMU_init(const MeasureGroup &meas, esekfom::esekf<state_ikfom, 12, input_ikfom> &kf_state, int &N)
{
  ...
  orient  << meas.imu.back()->orientation.x, meas.imu.back()->orientation.y, meas.imu.back()->orientation.z, meas.imu.back()->orientation.w;
  init_state.rot = SO3(Eigen::Quaterniond(orient));
  init_state.grav = S2(V3D(0, 0, -1*G_m_s2));
  
  //state_inout.rot = Eye3d; // Exp(mean_acc.cross(V3D(0, 0, -1 / scale_gravity)));
  // init_state.bg  = mean_gyr;  // dynamic init cannot use it.
